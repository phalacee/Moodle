...
/**
 * Definition of the grade_my_report class
 *
...
/**
 * Class providing an API for the My Grades report building and displaying.
 * @uses grade_report
 * @package gradereport_my
 */
class grade_report_my extends grade_report {
    ...
    public function __construct($userid, $gpr, $context) {
        ...
        if ($this->courses) {
            foreach ($this->courses as $course) {
                $this->showrank[$course->id] = grade_get_setting($course->id, 'report_my_showrank', !empty($CFG->grade_report_overview_showrank));
                if ($this->showrank[$course->id]) {
                    $this->showrank['any'] = true;
                }

                $this->showtotalsifcontainhidden[$course->id] = grade_get_setting($course->id, 'report_my_showtotalsifcontainhidden', $CFG->grade_report_overview_showtotalsifcontainhidden);
            }
        }


        // base url for sorting by first/last name
        $this->baseurl = $CFG->wwwroot.'/grade/my/index.php';
...
function grade_report_my_settings_definition(&$mform) {
    global $CFG;
    ...
    if (empty($CFG->grade_myreport_showrank)) {
        $options[-1] = get_string('defaultprev', 'grades', $options[0]);
    } else {
        $options[-1] = get_string('defaultprev', 'grades', $options[1]);
    }

    $mform->addElement('select', 'report_my_showrank', get_string('showrank', 'grades'), $options);
    $mform->addHelpButton('report_my_showrank', 'showrank', 'grades');
    ...
    if (empty($CFG->grade_report_my_showtotalsifcontainhidden)) {
        $options[-1] = get_string('defaultprev', 'grades', $options[0]);
    } else {
        $options[-1] = get_string('defaultprev', 'grades', $options[1]);
    }

    $mform->addElement('select', 'report_my_showtotalsifcontainhidden', get_string('hidetotalifhiddenitems', 'grades'), $options);
    $mform->addHelpButton('report_my_showtotalsifcontainhidden', 'hidetotalifhiddenitems', 'grades');
}
